...
const GroupDetailsPage = () => {
  ...
  const { loading: personLoading, error: personError, data: personData } = useQuery(GET_PERSON_BY_AUTH_ID, {
  ...
  const handlePhotoChange = async (e: any) => {
		...
		try {
			...
			if (response.ok) {
				if (group.photo != '') {
					await fetch(
						`http://localhost:8000/delete-file?file_id=${group.photo}&token=${token}`,
						{
							method: 'DELETE',
						}
					);
  ...
  const { data:userdata, loading:usergloading, error:usergerror} = useQuery(GET_USER_PROFILE, {
    variables: { token: token },
  });
  ...
  useEffect(() => {
    if (personData && personData.PersonByAuthID) {
      const ownerId = personData.PersonByAuthID.id ? parseInt(personData.PersonByAuthID.id, 10) : null;
      setOwnerId(ownerId);
    }
  }, [personData]);
    ...
    <div className="container mx-auto px-6 py-10 ">
        ...
        <div className="flex items-center justify-between mb-6">
          <div className="flex items-center">
            ...
            />
            {isEditing ? (
						<form className='flex flex-col items-start space-y-4' onSubmit={handleSaveChanges}>
							<input
								type='text'
								name='name'
              ...
              <p className="mb-6 text-lg text-gray-400 ">{group.description}</p>
              {userId === ownerId && (
          ...
          <button 
            className={`bg-blue-500 hover:bg-blue-600 text-white font-semibold py-3 px-6 rounded-lg ${isMemberOrRequested() || userId === ownerId ? 'hidden' : ''}`}
      ...
      <div className='flex flex-col items-center'>
        <h1 className='flex text-3xl font-bold mb-4 items-center'>Creación de publicaciones</h1>
